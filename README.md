...
### v0.2.0
- ✅ 修复直播时画面伸缩问题
- ✅ 新增实时样式监听器，使用MutationObserver监听视频元素的样式变化
- ✅ 当直播视频重置transform样式时，立即恢复放大效果
- ✅ 消除定时器检查的延迟，确保直播画面保持稳定的放大状态
- ✅ 优化资源管理，确保样式监听器正确清理

### v0.1.1
- ✅ 优化暂停时维持放大效果的机制
- ✅ 改进暂停事件监听器逻辑，确保视频暂停时立即启动持续检查
- ✅ 优化持续检查定时器，每200毫秒检查一次放大状态
- ✅ 添加状态标志防止样式应用过程中的重复触发
- ✅ 确保视频暂停时放大效果保持稳定，不会丢失

### v0.1.0
- ✅ 修复F键快捷键与抖音冲突的问题
- ✅ 添加capture:true参数使事件在捕获阶段触发
- ✅ 使用e.stopPropagation()阻止事件冒泡到抖音监听器
- ✅ 确保F键功能正常工作，不受抖音默认行为干扰

### v0.0.14
- ✅ 优化暂停时维持放大倍数的机制
- ✅ 添加专门的暂停事件监听器，确保视频暂停时立即恢复放大效果
- ✅ 复用现有的放大状态检查逻辑，提高代码复用性

### v0.0.13
- ✅ 去除旧的维持放大倍数的逻辑，保留新的循环检查方式
- ✅ 实现循环定期检查，修复视频暂停失去放大功能的问题
...
### v0.0.10
- ✅ 修复空格键点击两次的bug
- ✅ 添加e.stopPropagation()阻止事件冒泡，防止浏览器默认点击行为与脚本冲突

### v0.0.9
- ✅ 添加空格键控制视频播放/暂停功能
- ✅ 只有在视频放大功能启用时才响应空格键事件，否则交由抖音自己处理
- ✅ 防止页面滚动等默认行为，确保空格键专注于控制视频
- ✅ 优化事件监听器管理，避免内存泄漏

### v0.0.8
- ✅ 修复直播时画面伸缩问题
- ✅ 优化样式保护器逻辑，减少不必要的触发
- ✅ 消除响应延迟，将所有setTimeout延迟改为使用requestAnimationFrame
- ✅ 优化过渡效果，在样式保护器触发时禁用过渡效果，避免视觉抖动
- ✅ 提高检测精度，改进isScaleModified函数，支持更多transform格式
- ✅ 添加直播场景检测，为直播页面应用专门的优化策略
- ✅ 提高脚本的性能和可靠性

### v0.0.7
- ✅ 修复按空格键暂停时放大倍数被取消的问题
- ✅ 在pause事件监听器中添加立即重新应用放大效果，确保空格键暂停时放大倍数不会被取消
- ✅ 使用捕获阶段的事件监听器，确保我们的处理在抖音之前执行
- ✅ 优化样式保护器的检测逻辑和响应速度，减少防抖动时间从30ms到10ms，确保更及时地响应样式变化
- ✅ 增强视频状态变化的处理，添加了seeking、seeked和ratechange事件监听器，确保视频状态变化时放大效果保持稳定
- ✅ 确保视频暂停后放大效果能够保持稳定，不会丢失
- ✅ 保持空格键暂停功能的正常工作，没有冲突

### v0.0.6
- ✅ 修复关闭面板后立即重新出现的问题
- ✅ 在关闭按钮点击事件中添加 e.stopPropagation()，阻止事件冒泡到切换按钮
